trigger SendEmailOnNewLead on Lead (before insert, before update) {
    ...
    for (Lead newLead : Trigger.new) {
        ...
        if (Trigger.isInsert || (Trigger.isUpdate && Trigger.oldMap.containsKey(newLead.Id) && newLead.Status != Trigger.oldMap.get(newLead.Id).Status)) {
            // Check if the Status field has changed to 'New' or 'Working'
            if ((newLead.Status == 'New' && !newLead.EmailOnNewSent__c) ||
